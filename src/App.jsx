...
// Debounce hook
// Note: Since we only trigger search on GO click, the debounce hook is technically unused
// but kept here for stability if future features reintroduce debouncing logic.
...
useEffect(() => {
  ...
  const mergeLocalHistoryToCloud = async (client, userId) => {
      ...
      // 尝试批量上传
      // 使用 upsert 避免重复
      // 注意：onConflict 字段必须是数据库中设置了 Unique Constraint 的字段组合！
      ...
      if (!error) {
          ...
          console.error("Batch merge failed: Needs Unique Constraint on (word, user_id).", error);
           // 如果合并失败，不清空本地数据
      }
